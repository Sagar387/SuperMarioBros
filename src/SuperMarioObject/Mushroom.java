...
public class Mushroom extends SuperMarioObject {
    ...
    public Mushroom(float x, float y, int scale, Handler handler, Player player) {
        super(x, y, ObjectID.mushroom, width, height, scale);
        this.handler = handler;
        this.player = player;
        sprite = tex.mushroom_1();
    	...
    	for (SuperMarioObject temp : handler.getGameObjs()) {
            if (temp.getID() == ObjectID.player) {
                if (getBounds().intersects(temp.getBounds())) {
                	...
                	Player.collectedMushroom = true;
                     Player player = (Player) temp;
                    player.setSpriteToLarge(); // Change the player's sprite to the large sprite
